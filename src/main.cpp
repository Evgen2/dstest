...
//  AUTHOR: Rob Tillaart
// VERSION: [0.2.6] - 2026-01-23

#include <OneWire.h>
#include <DS18B20.h>

const int DS1820_1 = 15; // D15 esp32
const int DS1820_2 = 26; // D26 esp32
...
{ public:
  ...
  DS18BTest(OneWire *_ds)
  {
    ds = _ds;
  }
  void loop_enumerate(void);
  bool read_scratchpad(uint8_t *addr, uint8_t *buff9);
  void loop_test_alarm(void);
  void loop_discover(void);
  void loop_classify(void);
...
/*******************************************/
DS18BTest ds1(&oneWire1), ds2(&oneWire2);
...
void setup()
{
  ...
  {
    Serial.println("Error: Could not find Tsensor1");
    while (1);
  }
  ...
  start = millis();
  while (!Tsensor1.isConversionComplete());
  stop = millis();

  Serial.print(stop - start);
  Serial.print("\t");
  Serial.println(Tsensor1.getTempC(), 2);
  Serial.println();

  if (Tsensor2.begin() == false)
  {
    Serial.println("Error: Could not find Tsensor2");
    while (1);
  }
  ...
  start = millis();
  while (!Tsensor2.isConversionComplete());
  stop = millis();

  Serial.print(stop - start);
  Serial.print("\t");
  Serial.println(Tsensor2.getTempC(), 2);
...
{ static int intest = 0;
  ...
  Comm.println(F("  4. Continuous read test"));
  Comm.println();
  ...
  for (;;) {
    ...
    if (Comm.available()) {
      ...
      { Comm.printf(" pin %d\n", DS1820_1);
        ds1.loop_enumerate();
        Comm.printf(" pin %d\n", DS1820_2);
        return ds2.loop_enumerate();
      }
      if (select == '1')
      { Comm.printf(" pin %d\n", DS1820_1);
        ds1.loop_test_alarm();
         Comm.printf(" pin %d\n", DS1820_2);
        return ds2.loop_test_alarm();
      }
      if (select == '2')
      { Comm.printf(" pin %d\n", DS1820_1);
          ds1.loop_discover();
         Comm.printf(" pin %d\n", DS1820_2);
       return ds2.loop_discover();
      }
      if (select == '3')
      { Comm.printf(" pin %d\n", DS1820_1);
        ds1.loop_classify();
         Comm.printf(" pin %d\n", DS1820_2);
        return ds2.loop_classify();
      }
      if (select == '4')
      { intest = 1;
        return;
...
//  for (int res = 9; res < 13; res++)
  {
    Tsensor1.setResolution(res);
    Tsensor2.setResolution(res);
...
//    delay(10);
    while (!Tsensor1.isConversionComplete());
    temperature = Tsensor1.getTempC();
    stop = millis();
    
    Serial.printf("%f", temperature);
  
//    Serial.printf("\tdt=%d\t%f", stop - start, temperature);
    start = millis();
    Tsensor2.requestTemperatures();
//    delay(10);
    while (!Tsensor2.isConversionComplete());
    temperature = Tsensor2.getTempC();
    stop = millis();
//  Serial.printf("\tdt=%d\t%f\n", stop - start, temperature);
    Serial.printf(" %f\n", temperature);
...
bool DS18BTest::read_scratchpad(uint8_t *addr, uint8_t *buff9) {
  ds->reset();
  ds->select(addr);
...
void DS18BTest::loop_test_alarm() {
...
void DS18BTest::loop_discover() { // this is the safe choice
...
int16_t DS18BTest::time_conversion(uint8_t *addr) {
...
void DS18BTest::send_reset(uint8_t *addr) {
  ds->reset();
  ds->select(addr);
  ds->write(0x64);
  delay(10);
...
uint8_t DS18BTest::one_byte_return(uint8_t *addr, uint8_t cmd) {
  ds->reset();
  ds->select(addr);
  ds->write(cmd);
  return ds->read();
...
void DS18BTest::n_byte_return(uint8_t *addr, uint8_t cmd, int count, uint8_t *buffer) {
  ds->reset();
  ds->select(addr);
  ds->write(cmd);
...
void DS18BTest::get_trim_A(uint8_t *addr, uint8_t *trim1, uint8_t *trim2) {
  ds->reset();
...
void DS18BTest::set_trim_A(uint8_t *addr, uint8_t trim1, uint8_t trim2) {
  ds->reset();
  ds->select(addr);
  ds->write(0x95);
  ds->write(trim1);
...
void DS18BTest::get_trim_params_A(uint8_t *addr, uint16_t *offset_param_11bit, uint8_t *curve_param_5bit) {
  uint8_t trim1, trim2;
  get_trim_A(addr, &trim1, &trim2);
  trim2param(trim1, trim2, offset_param_11bit, curve_param_5bit);  
}

void DS18BTest::set_trim_params_A(uint8_t *addr, uint16_t offset_param_11bit, uint8_t curve_param_5bit) {
  uint8_t trim1, trim2;
...
void DS18BTest::trigger_convert(uint8_t *addr, uint8_t conf, uint16_t wait){
  ds->reset();
  ds->select(addr);
  ds->write(0x4E);
  ds->write(0xaf);
...
int DS18BTest::offset_param_range(uint8_t *addr) {
...
int DS18BTest::curve_param_prop(uint8_t *addr) {
